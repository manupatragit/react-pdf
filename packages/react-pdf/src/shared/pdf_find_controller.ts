...
export class PDFFindController {
  ...
  #reset() {
    ...
    this._firstPageCapability = new pdfjs.PromiseCapability();
  }
  ...
  #shouldDirtyMatch(state) {
    ...
    } else if (/* isArray && */ JSON.stringify(newQuery) !== JSON.stringify(prevQuery)) {
      return true;
    }
  ...
  #extractText() {
    ...
    for (let i = 0, ii = this._linkService.pagesCount; i < ii; i++) {
      const extractTextCapability = new pdfjs.PromiseCapability();
